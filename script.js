...
function loadTribos() {
    ...
    if (container) {
        ...
        // Garantir que appData.tribos existe e é um array
        if (!Array.isArray(appData.tribos)) {
            appData.tribos = [];
        }
        ...
        if (appData.tribos.length === 0) {
            ...
            console.log('Carregando', appData.tribos.length, 'tribos');
...
function saveTribo(event, id = null) {
    ...
    console.log('Iniciando salvamento de tribo, ID:', id);
    
    // Garantir que appData.tribos existe
    if (!Array.isArray(appData.tribos)) {
        console.log('Inicializando array de tribos');
        appData.tribos = [];
    }
    ...
    const tribo = {
        ...
        criadoEm: id ? (appData.tribos.find(t => t.id === id)?.criadoEm || new Date().toISOString()) : new Date().toISOString(),
        atualizadoEm: new Date().toISOString()
    };
    
    console.log('Dados da tribo:', tribo);
    
    if (id) {
        const index = appData.tribos.findIndex(t => t.id === id);
        if (index !== -1) {
            appData.tribos[index] = tribo;
            console.log('Tribo atualizada no índice:', index);
            showToast('Tribo atualizada com sucesso!', 'success');
        } else {
            console.error('Tribo não encontrada para atualização:', id);
            showToast('Erro: Tribo não encontrada para atualização', 'error');
            return;
        }
    } else {
        appData.tribos.push(tribo);
        console.log('Nova tribo adicionada. Total de tribos:', appData.tribos.length);
